...
export class ExperienceInterpreter implements SectionInterpreter<Experience> {
  readonly sectionName = 'experience'
  readonly urlPath = 'details/experience'

  interpret(item: ExtractedItem): Experience | null {
    const texts = item.texts.map((t) => t.trim()).filter(Boolean)
    if (texts.length === 0) return null

    if (item.subItems && item.subItems.length > 0) {
      const company = texts[0] ?? undefined
      const positions = item.subItems
      ...
      return {
        company,
        companyUrl: item.links[0]?.url,
        plainText: toPlainText(item.texts),
        positions,
      }
    }

    const parsed = parseSingleExperience(texts)
...
function extractMeta(lines: string[]): {
  ...
  return { fromDate, toDate, duration, location, description }
}
